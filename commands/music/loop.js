module.exports.run = async (client, msg, args) => {
  ...
  if(!serverQueue) return msg.channel.send({ color: 0x4db2b6, embed: { description: `Im not playing anything right now`}});
		if(!msg.channel.send) return msg.channel.send({ color: 0x4db2b6, embed: { description: `You must join voice channel to loop/unloop queue`}});
  if (serverQueue.voiceChannel.id !== msg.member.voiceChannel.id) return msg.channel.send({ embed: { color: 0x8bb6ff, description: `You must be in **${serverQueue.voiceChannel.name}** to loop the queue`}});
		...
		return msg.channel.send({ color: 0x4db2b6, embed: { description: `${serverQueue.loop ? 'Loop' : 'Unloop' } current queue`}});
